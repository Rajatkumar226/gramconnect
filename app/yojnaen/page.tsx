...
const categories = ["All", "Agriculture", "Employment", "Health", "Housing", "Women", "Education", "Social Security"];
...
export default function YojnaenPage() {
  ...
  const filtered = schemes.filter((s) => {
    ...
    const matchQ = !str || s.name.toLowerCase().includes(str) || s.eligibility.toLowerCase().includes(str) || s.benefit.toLowerCase().includes(str);
  ...
  return (
    <div style={{ backgroundColor: "var(--bg)", minHeight: "100vh" }}>
      ...
      <div className="relative py-20 sm:py-28 overflow-hidden" style={{ background: "linear-gradient(160deg, #041A0C 0%, #0C2E1A 45%, #163A24 100%)" }}>
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
            ...
            <span className="text-xs font-semibold tracking-widest uppercase" style={{ color: "#74C69D" }}>Government Schemes</span>
          </motion.div>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.1 }}
            ...
            Sarkari <span className="gold-shimmer">Yojnaen</span>
          </motion.h1>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }}
            className="text-sm mb-3 font-hindi" style={{ color: "rgba(255,255,255,0.45)" }}>
            सरकारी योजनाएँ — केंद्र व हिमाचल प्रदेश
          </motion.p>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }}
            className="text-base max-w-lg mx-auto" style={{ color: "rgba(255,255,255,0.55)" }}>
            {schemes.length}+ curated schemes for HP/Kangra residents — eligibility, documents & official links.
          </motion.p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pb-8 sm:pb-20">
        {/* Search */}
        <motion.div {...fadeUp()} className="-mt-5 mb-5">
        ...
        <motion.div {...fadeUp(0.06)} className="flex gap-2 flex-wrap mb-6">
          {categories.map((c) => (
            <motion.button key={c} onClick={() => setCat(c)}
              ...
              style={{
                background: cat === c ? "linear-gradient(135deg, var(--green), var(--green-mid))" : "var(--card)",
                color: cat === c ? "white" : "var(--text-2)",
                border: `1px solid ${cat === c ? "transparent" : "var(--border)"}`,
                boxShadow: cat === c ? "0 4px 12px rgba(27,67,50,0.25)" : "none",
              }}
            >
        ...
        <motion.p {...fadeUp(0.08)} className="text-sm mb-5" style={{ color: "var(--text-3)" }}>
          <strong style={{ color: "var(--green)" }}>{filtered.length}</strong> scheme{filtered.length !== 1 ? "s" : ""}
          {cat !== "All" && <> in {cat}</>}
        </motion.p>
        ...
        <div className="flex flex-col gap-3">
          {filtered.map((s, i) => (
            <motion.div key={s.id} {...fadeUp(i * 0.04)} className="card overflow-hidden">
              <button className="w-full text-left p-4 sm:p-5"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-start gap-2 mb-1">
                      <h3 className="font-semibold text-sm sm:text-base" style={{ color: "var(--text)" }}>{s.name}</h3>
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium"
                        style={{ backgroundColor: levelBadge[s.level]?.bg || "rgba(100,100,100,0.1)", color: levelBadge[s.level]?.text || "var(--text-3)" }}>
                        {s.level}
                      </span>
                    </div>
                    <p className="text-xs font-hindi mb-1.5" style={{ color: "var(--gold)" }}>{s.nameHi}</p>
                  ...
                  >
                    <div className="px-4 sm:px-5 pb-4 sm:pb-5 pt-3 border-t" style={{ borderColor: "var(--border)" }}>
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        >
                          Apply / View <ExternalLink size={12} />
                        </motion.a>
                      </div>
                    </div>
                  </motion.div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-14" style={{ color: "var(--text-3)" }}>
              ...
              <p className="font-medium" style={{ color: "var(--text-2)" }}>No schemes found</p>
